...
function nectar_main_styles() {
  ...
  if( $header_secondary_format === 'header_with_secondary') {
	 ...
	 // Testimonials.
	 if ( nectar_using_content(array('[testimonial_slider','[nectar_single_testimonial')) ) {
		 wp_enqueue_style( 'nectar-element-testimonial' );
	 }
	 	 
	 // Image with hotspots.
	 if ( nectar_using_content(array('[nectar_image_with_hotspots')) ) {
		 wp_enqueue_style( 'nectar-image-with-hotspots' );
	 }
	 
	 // Fancy box.
	 if ( nectar_using_content(array('[fancy_box')) ) {
		 wp_enqueue_style( 'nectar-element-fancy-box' );
	 }
	 
	 // Flip box.
	 if ( nectar_using_content(array('[nectar_flip_box')) ) {
		 wp_enqueue_style( 'nectar-element-flip-box' );
	 }
	 
	 // Category grid.
	 if ( nectar_using_content(array('[nectar_category_grid')) ) {
		 wp_enqueue_style( 'nectar-element-category-grid' );
	 }
	 
	 // Icon list.
	 if ( nectar_using_content(array('[nectar_icon_list')) ) {
		 wp_enqueue_style( 'nectar-element-icon-list' );
	 }
	 
	 // Tabbed section.
	 if ( nectar_using_content(array('[tabbed_section')) ) {
		 wp_enqueue_style( 'nectar-element-tabbed-section' );
	 }
	 
	 // Team member.
	 if ( nectar_using_content(array('[team_member')) ) {
		 wp_enqueue_style( 'nectar-element-team-member' );
	 }
	 
	 // Pricing table.
	 if ( nectar_using_content(array('[pricing_table')) ) {
		 wp_enqueue_style( 'nectar-element-pricing-table' );
	 }
	 
	 // Column border.
	 if ( nectar_using_content(array('column_border_color="#','column_border_color="rgb')) ) {
		 wp_enqueue_style( 'nectar-element-wpb-column-border' );
	 }
	 
	 // Recent posts.
	 if ( nectar_using_content(array('[recent_posts')) || is_page_template( 'template-home-2.php' ) || is_page_template( 'template-home-3.php' ) ) {
...
function nectar_page_sepcific_styles() {
	...
	// Nectar slider.
	if ( nectar_using_content(array('[nectar_slider','type="nectarslider_style"')) ) {
		wp_enqueue_style( 'nectar-slider' );
	}

	// Portfolio.
	if ( nectar_using_content(array('nectar_portfolio','recent_projects', 'type="image_grid"')) ||
	...
	if ( nectar_using_content(array('[nectar_blog')) && nectar_using_content(array('layout="std-blog-')) && nectar_using_content(array('blog_standard_style="classic')) ||
		nectar_using_content(array('[nectar_blog')) && nectar_using_content(array('layout="std-blog-')) && nectar_using_content(array('blog_standard_style="minimal')) ) {
		wp_enqueue_style( 'nectar-portfolio' );
	}
	...
	if ( nectar_using_content(array('.svg')) && nectar_using_content(array('Extra-Color-Gradient')) ) {
		wp_enqueue_style( 'linea' );
	}
