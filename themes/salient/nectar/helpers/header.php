...
function nectar_body_attributes() {
	...
	$body_attributes_escaped .= 'data-user-account-button="' . esc_attr( $user_account_btn ) . '" ';
...
if ( ! function_exists( 'nectar_page_trans_markup' ) ) {
	...
	function nectar_page_trans_markup() {
		...
		$nectar_transition_markup_escaped = '';

		$nectar_transition_markup_escaped .= '<div id="ajax-loading-screen" data-disable-mobile="' . esc_attr( $nectar_disable_transition_on_mobile ) . '" data-disable-fade-on-click="' . esc_attr( $nectar_disable_fade_on_click ) . '" data-effect="' . esc_attr( $page_transition_effect ) . '" data-method="standard">';

		if ( $page_transition_effect === 'horizontal_swipe' || $page_transition_effect === 'horizontal_swipe_basic' ) {

				$nectar_transition_markup_escaped .= '<div class="reveal-1"></div>';
				$nectar_transition_markup_escaped .= '<div class="reveal-2"></div>';

		} elseif ( $page_transition_effect === 'center_mask_reveal' ) {

			 $nectar_transition_markup_escaped .= '<span class="mask-top"></span>';
			 $nectar_transition_markup_escaped .= '<span class="mask-right"></span>';
			 $nectar_transition_markup_escaped .= '<span class="mask-bottom"></span>';
			 $nectar_transition_markup_escaped .= '<span class="mask-left"></span>';

		} else {

			 $nectar_transition_markup_escaped .= '<div class="loading-icon ' . $nectar_loading_image_animation_class . '">';
			...
			if ( empty( $loading_img ) ) {
				...
				if ( $loading_icon === 'material' ) {
					...
					$nectar_transition_markup_escaped .= '<div class="material-icon">
									 <div class="spinner">
					...
					if ( ! empty( $nectar_options['theme-skin'] ) && $nectar_options['theme-skin'] === 'ascend' ) {
							$nectar_transition_markup_escaped .= '<span class="default-loading-icon spin"></span>';
					} else {
							$nectar_transition_markup_escaped .= '<span class="default-skin-loading-icon"></span>';
					}
				}
			} // empty loading img

				$nectar_transition_markup_escaped .= '</div>';

		} // not swipe or mask reveal

		$nectar_transition_markup_escaped .= '</div>';

		echo $nectar_transition_markup_escaped; // WPCS: XSS ok.
...
if ( ! function_exists( 'nectar_header_social_icons' ) ) {
	...
	function nectar_header_social_icons( $location ) {
		...
		if ( $location === 'secondary-nav' ) {
			$social_output_html_escaped .= '<ul id="social">';
		}
		...
		foreach ( $social_networks as $network_name => $icon_class ) {
			...
			if ( $network_name === 'rss' ) {
				if ( ! empty( $nectar_options[ 'use-' . $network_name . '-icon-header' ] ) && $nectar_options[ 'use-' . $network_name . '-icon-header' ] === '1' ) {
					...
					$social_output_html_escaped .= $social_link_before . '<a target="_blank" href="' . esc_url( $nectar_rss_url_link ) . '"><i class="' . $icon_class . '"></i> </a>' . $social_link_after;
				}
			} else {
				$target_attr = ($network_name != 'email' && $network_name != 'phone') ? 'target="_blank"' : '';
				if ( ! empty( $nectar_options[ 'use-' . $network_name . '-icon-header' ] ) && $nectar_options[ 'use-' . $network_name . '-icon-header' ] === '1' ) {
					$social_output_html_escaped .= $social_link_before . '<a '.$target_attr.' href="' . esc_url( $nectar_options[ $network_name . '-url' ] ) . '"><i class="' . $icon_class . '"></i> </a>' . $social_link_after;
				}
			}
		}

		if ( $location === 'secondary-nav' ) {
			$social_output_html_escaped .= '</ul>';
		}

		echo $social_output_html_escaped; // WPCS: XSS ok.
	}
}
